...
random_path = os.environ.get("BASE_PATH", "".join(random.choice(string.ascii_letters + string.digits) for _ in range(32)))
conf = {}
...
class Gitlab:
    ...
    def create_issue(self, domain, body):
        try:
            new_issue = self.project.issues.create({"title": domain, "description": body})
            return new_issue.iid
        except Exception as e:
            print(e)
        return 0

    def create_issue_note(self, body, issue_iid):
        try:
            self.project.issues.get(issue_iid).notes.create({"body": body})
        except Exception as e:
            print(e)
    ...
    def do_POST(self):
        ...
        if self.path == f"/{random_path}/hunting":
            try:
                ...
                for report in body["reports"]:
                    domain = report["domain"]
                    issue_iid = gl.create_issue(domain, create_md_report(report))
                    obs = get_observation()
                    used = len(obs["observation_urls"].keys())
                    if LIMIT > used:
                        add_observation(domain, comment=json.dumps({"issue_iid": issue_iid}))
                    url = f"https://{domain}"
        ...
        if self.path == f"/{random_path}/observation":
            try:
                ...
                for report in body["reports"]:
                    domain = report["domain"]
                    comment = json.loads(report["comment"])
                    issue_iid = comment.get("issue_iid")
                    if issue_iid:
                        gl.create_issue_note(create_md_report(report), issue_iid)
                body = "OK"
                statusCode = 200
            except Exception as e:
                body = str(e)
